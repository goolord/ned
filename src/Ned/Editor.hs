...
editorFrame focused rect cellW fm ed0 = do
  ...
  let g = geometry cellW fm buf1
      ...
      -- The line and the offset under the pointer, with the view scrolled to
      -- a line. The pointer takes nothing but a selection, so it looks
      -- through the text as the keys left it.
      pointedLine scrollY = clamp 0 (B.lineCount buf1 - 1) (floor (scrollY + realToFrac (localY / gLineH g)))
      pointed scrollY =
        let x = localX - gGutterW g - textPad + edScrollX ed0
         in B.offsetAt buf1 (pointedLine scrollY) (round (x / gCellW g))
      bar = scroller g rowRect buf1
      hbar = hscroller g rowRect buf1
  ...
  let (drag1, buf2, scrollY1)
        ...
        | inputMousePressed inp && overGutter =
            ...
            let ln = pointedLine (edScrollY ed0)
        ...
        | inputMouseRightPressed inp && inside && not overBar =
            ...
            let off = if overGutter then B.lineStart buf1 (pointedLine (edScrollY ed0)) else pointed (edScrollY ed0)
        ...
        | otherwise = case edDrag ed0 of
            ...
            DragSelect -> (DragSelect, B.setCursor True (pointed edgeScrolled) buf1, edgeScrolled)
      ...
      scrollX3 = clamp 0 (maxScrollX g rowRect buf2) (followX scrollX2)

  when autoScrolling (wakeAfter 0.03)

  -- The lexer state the first line on screen starts in.
  let lexCache = lexCacheFor ed0 buf0 buf2 (floor scrollY3)
